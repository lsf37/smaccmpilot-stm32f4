...
module SMACCMPilot.SafePack (
  ...
  -- * Unpacking
  munpack, unpackFrom, unpackFrom_
) where
...
mpack ref = PackM $ do
  ...
  if new_offset > arrayLen arr
    then error $ "packing " ++ (show new_offset) ++ " bytes into "
              ++ "array of length " ++ (show (arrayLen arr :: Int))
    else return ()
  lift $ lift $ call_ pack (toCArray arr) (fromIntegral offset) val
  set new_offset
...
munpack ref = UnpackM $ do
  ...
  lift $ lift $ do
    val <- call unpack (toCArray arr) (fromIntegral offset)
    store ref val
  set new_offset

-- XXX define marrayUnpack
